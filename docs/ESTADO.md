...
**Última actualización:** 2026-06-15 01:20 UTC.
...
10. ✅ **Fase 4 — extensiones Chrome/Firefox (`extension/`)**: extensión cliente, **patrón (a)** del plan (la extensión NO scrapea; el popup pega contra el FastAPI local).
   ...
   - **`entrypoints/popup/`**: UI vanilla TS (form cuentas+fechas → `POST /jobs` → polling de `GET /jobs/{id}` cada 1 s → muestra estado/log/gate y links de descarga por cuenta). **`entrypoints/background.ts`**: service worker mínimo; **auto-arranca el backend vía Native Messaging** (ver ítem 12). Recuerda la URL base en `browser.storage.local`.
...
11. ✅ **Patrón C — captura GraphQL in-page** (la respuesta al bloqueo de twscrape; spec/plan `2026-06-14-inpage-capture*`). La extensión captura el GraphQL que x.com YA descarga (el navegador calcula el `x-client-transaction-id`); el backend lo ingiere reusando el pipeline. **Esquiva de raíz el anti-bot que dejó muerto al scraping server-side.**
   - **Backend** (Tasks 1-5): `SlidingWindowGate.record(n)` (contabilidad *record-after*: el acceso ya ocurrió en el browser → se registra sin reservar; over-cap → 429 a futuras ingestas; aprobado por el usuario) · `providers/search_envelope.py` (walkers de envelope neutrales, sin twscrape) · `export_account`/`iter_account` filtran por `[since, until)` · `POST /ingest` (extract → map → `store.save` dedup → `gate.record`; RT descartado pero contado) · `POST /export` + `GET /exports/{file}` (CSV on-demand a `captures_dir`, guard anti-traversal).
   - **Extensión** (Task 6): `lib/capture.ts` (puro: `matchOp` por nombre de op, `accountFromUrl` desde `from:user`) · content script `capture-main` (world **MAIN**, parchea fetch/XHR sobre un clon, nunca rompe la página) → `capture-bridge` (ISOLATED) → `background` (batchea por cuenta → `POST /ingest`, buffer+reintento; 429 no reintenta; contadores en storage) · popup con toggle de captura, contadores y form de export. La extensión NO parsea GraphQL (regla del repo): el backend hace todo.
   - **Crawl adaptativo + auto-scroll** (`autoscroll.content.ts` + `lib/autoscroll.ts`, permiso `activeTab`): el popup tiene UN form (cuenta + desde + hasta) con **Capturar / Detener / Exportar**. "Capturar" escribe `STORAGE_CRAWL = {account, since, until}` (rango COMPLETO) y abre la búsqueda (`searchUrl`, `f=live`) en una pestaña nueva. El content script de esa pestaña **solo arranca si la página ES la búsqueda en curso** (`matchesSearch`, evita secuestrar otra pestaña), scrollea al fondo y, cuando deja de crecer, **decide en `onSettled`**: si el tweet más viejo cargado ya alcanzó `since` → terminó; si se frenó ANTES (la pared de ~1000 del search en vivo, o un error de paginación) → **encoge `until` al día del más viejo** (`nextUntil`, incluye ese día → solapa, el store deduplica; cae a exclusivo si un solo día satura, para garantizar avance) y re-navega más viejo, hasta cubrir todo `[since, until)`. **Sin ventanas fijas: se adapta a la densidad real.** Robustez de carga: si hay spinner **espera** (no cuenta estancado), si X muestra **"Reintentar"** lo clickea (transitorio se recupera; pared dura persiste → narrow), si carga **en blanco** recarga acotado y, si no hay caso, corta dejando un aviso en `STORAGE_STATUS` que el popup muestra al reabrirse. El día del tweet más viejo se lee del `time[datetime]` ya renderizado — **navegación, no parsing de GraphQL** (el backend sigue siendo el único que mapea). "Detener" borra el crawl (no narrowa más) y mensajea la pestaña. El form se persiste en `STORAGE_FORM` y se restaura al reabrir el popup (sin esto, "Exportar" no encontraba cuenta/fechas y cortaba en silencio). Hands-free: ingresás cuenta+fechas → la extensión abre, scrollea y se va narrowando sola.
   - **Cumplimiento**: el gate sigue siendo el único ledger global 1M/24h; el camino in-page lo alimenta con `record` (registrar-después) en vez de `reserve` (reservar-antes-y-bloquear). Ledger no deduplica.
   - **Verificado**: `tsc` + `wxt build` (chrome+firefox), 6+3 tests de `/ingest`+`/export`. **NO verificado**: el flujo end-to-end cargando la extensión en un navegador real (no automatizable acá).

12. ✅ **Auto-arranque del backend vía Native Messaging (`deploy/native-host/`)** — elimina el "tener un `uvicorn` corriendo a mano en WSL". El navegador arranca y baja el FastAPI solo; **no reemplaza el servicio HTTP, lo supervisa** (gate/`mappers`/`storage` siguen server-side, la extensión sigue pegando HTTP a `localhost:8000`).
   - **Cómo**: `background.ts` abre `connectNative('com.xport.host')` al cargar y **mantiene el puerto** (= backend vivo durante la sesión); el host `xport-host.sh` (`uv run` → venv) corre `host.py` (supervisor stdlib) que lanza uvicorn, espera `/healthz` y, en **EOF del puerto** (navegador cerrado), lo baja. **Idempotente**: si ya hay un server, no arranca otro ni lo mata. uvicorn corre desde la raíz del repo → **el MISMO `data/audit/ledger.db`**: el gate sigue siendo el único ledger global, ningún camino lo saltea.
   - **Instalación** (una vez, fuera del navegador — native messaging no se shippea por la store): `install-linux.sh` (Firefox/Chrome dentro de WSL/WSLg) · `install-windows.ps1` (Firefox en Windows → WSL vía `wsl.exe`). Permiso `nativeMessaging` + `gecko.id = xport@local` (debe matchear el manifest del host). Si el host no está instalado, el background cae al server manual (los `/ingest` reintentan). **Chrome MV3**: SW efímero → usar `XPORT_HOST_KEEP_ALIVE=1`; **Firefox MV2** anda directo. Ver `deploy/native-host/README.md`.
   - **Verificado** (en WSL): test de ciclo de vida `arranque → /healthz 200 → EOF → uvicorn baja`, stdout del host vacío (no corrompe el canal nativo), e idempotencia (no mata un uvicorn manual) → **PASS**; `tsc` + `wxt build` OK con `nativeMessaging`+id en ambos manifests. **NO verificado**: el handshake real Firefox→host en un navegador, en particular el bridge **Windows→WSL vía `wsl.exe`** (no automatizable acá).

**Calidad actual:** 222 tests Python verdes · `mypy --strict` limpio (30 archivos) · `ruff` (lint+format) limpio · build Docker Alpine OK · extensión: `tsc` + `wxt build` (chrome+firefox) OK. En `main`.
...
1. **Verificar la captura in-page (patrón C) end-to-end en un navegador real** — el camino elegido tras el bloqueo de twscrape, **ya implementado en código** (ítem 11): cargar la extensión, correr el servicio, abrir el popup, ingresar **cuenta + desde + hasta** y darle **"Capturar ▶"** (la extensión abre la búsqueda y scrollea sola), y confirmar que `/ingest` persiste + el gate sube + **"Exportar CSV"** baja el archivo. (El scraping server-side vía twscrape sigue **bloqueado** por el anti-bot de x.com — ver "Verificación con datos vivos" abajo —; C lo esquiva.)
...
Mejoras de la extensión ya hechas: patrón (c) captura GraphQL in-page (ítem 11) y patrón (b) Native Messaging — auto-arranque del backend sin server a mano (ítem 12, falta verificar el bridge Windows→WSL en un navegador real). Ver `docs/plan-extractor-tweets.md`.
...
**✅ IMPLEMENTADO (2026-06-14): patrón C completo en código** (las 7 tasks del plan; ver ítem 11 de "Qué está hecho"). El bloqueo de twscrape queda **respondido** por esta vía. Pendiente solo la verificación end-to-end cargando la extensión en un navegador real.

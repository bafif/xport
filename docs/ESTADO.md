...
**Última actualización:** 2026-06-14 18:20 UTC.
...
1. **Verificar la captura in-page (patrón C) end-to-end en un navegador real** — el camino elegido tras el bloqueo de twscrape, **ya implementado en código** (ítem 11): cargar la extensión, correr el servicio, navegar `x.com/search?q=from:user since: until:&f=live`, scrollear, y confirmar que `/ingest` persiste + el gate sube + `/export` baja el CSV. (El scraping server-side vía twscrape sigue **bloqueado** por el anti-bot de x.com — ver "Verificación con datos vivos" abajo —; C lo esquiva.)

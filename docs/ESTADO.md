...
**Última actualización:** 2026-06-15 00:50 UTC.
...
11. ✅ **Patrón C — captura GraphQL in-page** (la respuesta al bloqueo de twscrape; spec/plan `2026-06-14-inpage-capture*`). La extensión captura el GraphQL que x.com YA descarga (el navegador calcula el `x-client-transaction-id`); el backend lo ingiere reusando el pipeline. **Esquiva de raíz el anti-bot que dejó muerto al scraping server-side.**
   ...
   - **Auto-scroll** (`autoscroll.content.ts` + `lib/autoscroll.ts`, permiso `activeTab`): botón en el popup que le manda start/stop al content script de la pestaña activa; éste scrollea la búsqueda hasta el fondo en loop (corta cuando la altura deja de crecer = fin de resultados, o al tope de pasos), disparando los fetches que captura-main intercepta. Hands-free: el usuario ya no scrollea a mano; puede cerrar el popup y sigue solo.
...
1. **Verificar la captura in-page (patrón C) end-to-end en un navegador real** — el camino elegido tras el bloqueo de twscrape, **ya implementado en código** (ítem 11): cargar la extensión, correr el servicio, abrir `x.com/search?q=from:user since: until:&f=live`, darle **"Auto-scroll ▶"** en el popup (ya no hace falta scrollear a mano), y confirmar que `/ingest` persiste + el gate sube + `/export` baja el CSV. (El scraping server-side vía twscrape sigue **bloqueado** por el anti-bot de x.com — ver "Verificación con datos vivos" abajo —; C lo esquiva.)

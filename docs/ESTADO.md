...
11. ✅ **Patrón C — captura GraphQL in-page** (la respuesta al bloqueo de twscrape; spec/plan `2026-06-14-inpage-capture*`). La extensión captura el GraphQL que x.com YA descarga (el navegador calcula el `x-client-transaction-id`); el backend lo ingiere reusando el pipeline. **Esquiva de raíz el anti-bot que dejó muerto al scraping server-side.**
   ...
   - **Auto-scroll + abrir la búsqueda** (`autoscroll.content.ts` + `lib/autoscroll.ts`, permiso `activeTab`): el popup tiene UN form (cuenta + desde + hasta) con **Capturar / Detener / Exportar**. "Capturar" arma la URL `searchUrl(account, since, until)`, prende un flag `STORAGE_AUTOSTART` en storage y **abre la búsqueda en una pestaña nueva**; el content script de esa pestaña consume el flag al cargar y arranca el auto-scroll **solo** (no depende del popup, que se cierra al abrir la pestaña). El loop scrollea hasta el fondo, corta cuando la altura deja de crecer (fin de resultados) o al tope de pasos, disparando los fetches que captura-main intercepta. "Detener" mensajea la pestaña activa. Hands-free de punta a punta: ingresás cuenta+fechas → la extensión abre y scrollea sola.

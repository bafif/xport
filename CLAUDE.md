...
- **Todo camino de acceso pasa por el gate.** El `GatedProvider` envuelve a CUALQUIER `TweetProvider` (scraping u oficial): ningún fetch puede saltearlo (reserva-antes). La captura GraphQL in-page de la extensión (patrón C) **ya reporta sus accesos** vía `gate.record(n)` (*record-after*: el acceso ya ocurrió en el browser, se registra sin reservar). Cualquier camino futuro DEBE alimentar el mismo ledger global.
...
**El scraping server-side vía twscrape está BLOQUEADO** (2026-06-14): x.com migró su web client a Vite y rompió el cálculo del `x-client-transaction-id` que twscrape reverse-engineerea (0.18.1 es la última versión, sin fix). NO es un bug de xport (el provider/gate fallan cerrado correctamente). La respuesta elegida e implementada es el **patrón C — captura GraphQL in-page** (specs/plans `2026-06-14-inpage-capture*`): la extensión captura el GraphQL que x.com ya descarga (el navegador calcula el transaction-id) y lo manda a `POST /ingest`, que reusa `mappers/`+`storage/`. **La extensión NO parsea GraphQL.** El Compliance Gate cuenta estos accesos con `gate.record(n)` (contabilidad *record-after*: el acceso ya ocurrió en el browser → se registra sin reservar; over-cap → 429 a futuras ingestas), no con `reserve` (reservar-antes-y-bloquear, que se usa para el scraping activo). Export on-demand: `POST /export` + `GET /exports/{file}`. Ver `docs/ESTADO.md` para el detalle y la verificación pendiente.
